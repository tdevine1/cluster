package edu.fsuhpc.dsps.datahandlers;

import java.text.DecimalFormat;

import org.apache.commons.math3.util.FastMath;

import edu.fsuhpc.dsps.functions.IdealPulseFitter;
import edu.fsuhpc.dsps.functions.IdealPulseFunction;
import edu.fsuhpc.dsps.util.ArrayUtils;
import edu.fsuhpc.dsps.util.Constants;

/**
 * @author zennisarix
 */
public class DistributedCandidate
{
	/**
	 ...
	 * @param name
	 */
	public DistributedCandidate(String name)
	{
		this.name = name;
		initialize();
	}
	...
	private void initialize()
	{
		...
		survey = nameFields[0];
		mjd = Integer.parseInt(nameFields[1]);
		raDec = extractRADec(nameFields[2], survey);
		beam = Integer.parseInt(nameFields[3]);
		if (survey.equals("gbt350drift"))
		{
			centralFrequency = Constants.NU_GBT;
			channelBandwidth = Constants.BANDWIDTH_GBT;
			numChannels = Constants.NUM_CHANNELS_GBT;
		}
		if (survey.equals("aodrift") || Constants.SURVEY.equals("palfa"))
		{
	 ...
	 * @param start
	 */
	public DistributedCandidate(String name, int start)
	{
		this.name = name;
		initialize();
		startIndex = start;
			...
			double[][] snrsMax, double[][] times)
	{
		try
		{
			...
			fitter = new IdealPulseFitter(new double[] { centralFrequency,
					channelBandwidth, numChannels, 1.0, 1.0, snrPeakDM });
	...
	public String[] extractRADec(String name, String survey)
	{
		String[] raDec = new String[2];
		int stopIndex = (name.indexOf("+") > 0) ? name.indexOf("+")
				: name.indexOf("-");
		raDec = new String[] { name.substring(0, stopIndex),
				name.substring(stopIndex) };
		return raDec;
	}
}
