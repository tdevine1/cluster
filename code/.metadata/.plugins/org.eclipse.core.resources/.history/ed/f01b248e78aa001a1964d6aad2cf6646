...
private[tree] sealed class TreeEnsembleModel(
   ...
   */
  private def predictByVoting(features: Vector):  (Double, Double) = {
    var confidence = 0.0
    ...
    confidence = confidence / trees.length
    if (votes.maxBy(_._2)._1 == 0)
      confidence = 1 - confidence
    (votes.maxBy(_._2)._1, confidence)
  }
...
//   */  
//   private def predictByVotingWithConf(features: Vector): (Double, Double) = {
...
//   * @return predicted category from the trained model
//   */
//  def predictWithConf(features: Vector): (Double, Double) = {
//    (algo, combiningStrategy) match {
//      case (Classification, Vote) =>
//        predictByVotingWithConf(features)
//      case _ =>
...
//        if (prediction > 0.0) 1.0 else 0.0
   ...
   */
  def predict(features: RDD[Vector]): RDD[(Double, Double)] = features.map(x => predict(x))
