	...
	public FileData(File path, int maxDMs, boolean makeDats) throws Exception
	{
		if (Constants.SURVEY.equals("palfa"))
			name = path.getName().substring(0,
					path.getName().lastIndexOf("_singlepulse"));
		else
		...
		if (makeDats)
		{
			File datDir = new File(Constants.DAT_PATH + name);
		...
		inBuffer.readLine();
		// get first data line
		String lines = inBuffer.readLine() + "\n";
		...
		processDMChunk(lastDM, lines.split("\n"));
		if (lastDM < 1065)
			System.err.println("ERROR: " + name + "\tMaxDM = " + lastDM);
		inBuffer.close();
	...
	private void processDMChunk(double dm, String[] dmChunk)
	{
		try
		{
			...
			for (int i = 0; i < dmChunk.length; i++)
			{
				...
				times[snrIndex][i + 1] = time = Double
