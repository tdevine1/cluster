...
object SelfLearningMLRF {
  def main(args: Array[String]) {
    ...
    while (iteration < MAX_ITERATIONS && unlabeledLP.count > 0)
    {     
      // Label the highest confidence predictions
      val predictions = unlabeledLP.map { point =>
        val (prediction, confidence) = model.predictByVotingWithConf(point.features)
        if (confidence >= 0.9)
